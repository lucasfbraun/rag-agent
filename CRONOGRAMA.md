...
**Status:** 🟨 **Em recuperação — 10.499 de 11.273 pontos históricos (faltam ~774), 3ª tentativa de reingestão em andamento (Sessão 32, 2026-09-02).** Incidente original da Sessão 30 (2026-08-26): um bug na implementação do ticket 7 (reconciliação de índice) apagou os 11.273 pontos reais durante uma verificação — corrigido (ver abaixo), mas as duas primeiras tentativas de reingestão completa (Sessões 30/31) foram interrompidas antes de terminar (disco cheio, depois reinício do Docker) sem chegar aos 11.273 originais. Sem backup/snapshot configurado até a Sessão 31 (ver Fase 8, item adiantado). **Os documentos-fonte nunca foram tocados**, só o índice vetorial. Detalhe do incidente original: `docs/incidente_2026-08-26_reingestao_apagou_colecao.md`.

**⚠️ Achado real na Sessão 32, ainda sem correção retroativa:** auditoria da coleção mostrou que **quase metade dos 10.499 pontos atuais é redundante** (pasta de rede duplicada + PDF/DOCX do mesmo boletim indexados em dobro) — `ingest_catalog_directory()` já ganhou filtros pra não acumular mais duplicata daqui pra frente (ver Fase 2), mas a limpeza do que já está indexado é uma decisão pendente com o usuário (operação destrutiva na coleção de produção), tratada à parte da reingestão em andamento.
