...
**Status:** 🟨 Em andamento (tarefas 1-8/9 concluídas — schema, service, autenticação, autorização, proteção dos endpoints, restrição de campos sensíveis estruturados, administração de usuários via API e testes adicionais de auth/autorização/campos sensíveis. ✅ Frontend Streamlit com login corrigido — ver "Frontend: tela de login" abaixo)
