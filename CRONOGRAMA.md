...
**✅ Sessões 38–39 — treinamento do agente concluído:** três modalidades — correção, conhecimento e exemplo — em coleção Qdrant separada (`pu_treinamento`). Correção e conhecimento passam por aprovação; exemplo entra direto. A interface permite cadastrar, listar e aprovar itens, e um feedback negativo abre o formulário para escrever a resposta correta. Correções usam corte de similaridade mais rigoroso, escopo por produto/aplicação/fonte e não são aplicadas quando a pergunta cita outro código. Feedback negativo bruto deixou de ser instrução global. Ver `docs/spec_treinamento.md` e `PROGRESS.md`.
