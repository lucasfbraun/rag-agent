...
**🟨 Sessão 38 — item 4, treinamento do agente (backend concluído):** três modalidades — correção, conhecimento e exemplo — em coleção Qdrant SEPARADA (`pu_treinamento`), porque a reingestão do acervo apagaria o treinamento junto e ele não tem arquivo de origem para ser refeito. **Não é fine-tuning:** o aprendizado fica em dado, removível apagando uma linha. Correção e conhecimento passam por aprovação; exemplo entra direto (assimetria de dano). Cada tipo entra no prompt com papel explícito — exemplo marcado como FORMA e não fato, para o LLM não copiar os números dele. 437/438. **Validado ao vivo com `gpt-4o-mini`:** pendente não influencia, aprovado aparece como orientação interna, exemplo não vaza números (ressalva: a manchete da resposta pode sair sem atribuição). **Falta a tela.** Ver `docs/spec_treinamento.md` e `PROGRESS.md`.
...
**Status:** 🟨 Em andamento (identidade visual + PWA na Sessão 27; histórico persistente na Sessão 33; tela de usuários e **marca oficial** na Sessão 35; sem validação em navegador real)
