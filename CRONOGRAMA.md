...
**Status:** 🟨 Em andamento (tarefas 1-7/9 concluídas — schema, service, autenticação, autorização, proteção dos endpoints, restrição de campos sensíveis estruturados e administração de usuários via API. ✅ Frontend Streamlit com login corrigido — ver "Frontend: tela de login" abaixo)
