...
**Duas lacunas distintas, dois caminhos novos.** O acervo já era pesquisável por nome/família e por aplicação/tipo (Sessão 32). Faltavam as duas dimensões que o vendedor mais usa numa conversa técnica:

**1. Busca por valor de especificação (`backend/app/rag/spec_search.py`, novo).** A busca vetorial não representa grandeza: "hidroxila 180" e "hidroxila 34" geram vetores quase idênticos, então o top-k trazia trechos que *falam* de hidroxila com o valor errado — erro silencioso, com cara de resposta certa. O módulo novo lê a tabela de especificação por regra (20 propriedades canônicas: hidroxila, NCO, viscosidade, densidade, dureza, pH, sólidos, água, acidez, funcionalidade, relação de trabalho, os tempos de reatividade, resiliência, alongamento, tração, rasgo, temperatura), interpreta a pergunta ("acima de", "entre X e Y", "de 180") e varre o acervo INTEIRO — não um top-k, porque a pergunta é "todos os produtos com hidroxila 180" e um punhado de chunks daria uma contagem errada sem ninguém perceber. Quando nada casa, devolve junto a `faixa_no_acervo` (mínimo e máximo daquela propriedade em todo o acervo), para o agente responder "não há produto com hidroxila 180; o acervo vai de 20 a 415 mgKOH/g" em vez de um "não encontrei" seco. Exposto como ferramenta MCP `consultar_produtos_por_especificacao` E injetado direto no contexto quando a pergunta é claramente dessa natureza — depender só do tool calling deixaria a resposta errada nas vezes em que o modelo não chamasse a ferramenta.

**2. Consulta por seção do boletim (`backend/app/rag/doc_sections.py`, novo).** Um boletim vira vários chunks; perguntar "vantagens do AG 2032" trazia o chunk da tabela de especificação ou da FISPQ — produto certo, seção errada — e o agente respondia com o que tinha na mão. Sete seções canônicas mapeadas a partir dos cabeçalhos REAIS do acervo (especificações, características e vantagens, aplicação, perfil de reatividade, segurança e armazenamento, embalagens, informações complementares), com o vocabulário do vendedor de um lado ("como armazenar", "vem em tambor?", "qual a validade") e o do documento do outro ("SEGURANÇA E ARMAZENAMENTO" no boletim, "MANUSEIO E ARMAZENAMENTO" na FISPQ). A seção pedida vira filtro de recuperação dentro do produto citado, reordenação do contexto e instrução explícita ("responda a partir desta seção; se ela não estiver aqui, diga que não consta — não entregue outra seção no lugar").

**3. Leitura estruturada da tabela injetada no contexto.** A extração de PDF embaralha as colunas ("Índice de hidroxilas mgKOH/g 54,0 – 58,0 56,80 Viscosidade Brookfield a 25 °C cPs 7500 – 8500 7810") e o LLM troca número, unidade e propriedade. Todo contexto recuperado agora leva junto a leitura já resolvida propriedade→valor dos MESMOS documentos, sem tirar o texto bruto — o agente precisa poder conferir a evidência.

**Layouts reais cobertos.** A ordem das colunas muda de template para template no acervo (mín/máx/resultado, resultado/unidade/especificação, unidade/especificação/resultado, `±`, `<`), então o parser NÃO adivinha qual número é qual: coleta os valores da célula, trabalha com a faixa observada e guarda o trecho literal como evidência para o vendedor conferir.

**Bugs encontrados e corrigidos durante a implementação (todos com teste de regressão):**
- **Temperatura do ensaio virava valor da propriedade.** "Viscosidade, 25°C cPs 800 a 900" era lido como viscosidade 25 — e aí *qualquer* produto do acervo atenderia a uma busca por viscosidade baixa. A causa era de ordem: `°C` também é unidade válida, e detectar unidade antes de remover o qualificador de temperatura apagava justamente a marca que identificava o 25 como temperatura.
- **Expoente da unidade entrava como valor.** "g/cm³" normalizado vira "g/cm3"; sem remover a unidade antes de procurar números, esse `3` entrava como valor de densidade.
- **Tempo de reação lido errado (achado a partir da pergunta do usuário).** O acervo escreve os tempos no formato minuto/segundo com apóstrofo duplo (ex: 00'09'' a 00'12''), alternando crase, apóstrofo simples e aspa curva no mesmo documento. Lido como número solto, "Tempo de Cura 120'00''" viraria 120 e 0 em vez de 120 minutos. Agora tudo é convertido para segundos na leitura e na pergunta ("2 minutos" → 120 s), e exibido de volta em linguagem de chão de fábrica ("1 min 21 s"). A coluna chamada "Min." no cabeçalho não é mais herdada como unidade.
- **Menção não é cabeçalho.** "o produto tem vantagens frente ao concorrente" era classificado como a seção "Características e vantagens" e o trecho ia para o topo do contexto. A regra agora distingue título de texto corrido pelo que vem antes no texto original (fim de frase, início do trecho, ou algo sem letra minúscula) — o mesmo cuidado evita truncar a seção de armazenamento em "manter a embalagem fechada".
- **Código de produto virando valor de especificação.** "qual a hidroxila do AG 2032" poderia disparar uma busca por hidroxila = 2032 (que não existe em lugar nenhum) e devolver "não encontrei" para uma consulta de ficha trivial. Códigos já detectados são removidos antes de procurar o número, e pergunta com código nomeado não dispara a varredura.
- **Custo da varredura.** Normalizar caractere a caractere com `unicodedata.normalize` dominava o tempo da consulta; trocado por uma tabela de `str.translate` com cache — 4x mais rápido (~2 s para uma varredura de 10 mil chunks).

**Ajustes no que já existia:** nome de seção ("vantagens", "armazenamento", "reatividade") não vira mais palavra-chave genérica do RAG — como termo solto batia em quase todo boletim e só enchia o top-k de ruído; agora tem caminho próprio, mais preciso. O prompt do agente ganhou a Situação D (pedido por especificação numérica) e a Situação A foi ampliada para cobrir qualquer assunto do produto, não só especificação.
...
**Correção complementar após novo relato do usuário:** pedir diretamente "liste os produtos elastômeros" ainda devolvia 89 itens, incluindo ADT e CAT, porque a ferramenta tratava qualquer menção a elastômero como natureza do produto. Evidência real: ADT 432 é "aditivo para elastômeros"; CAT 100 é catalisador usado na produção; TH T160DE1 é o pré-polímero que, combinado com o curativo, "produz elastômero de poliuretano". Para esse tipo, a classificação agora exige evidência positiva no Boletim (produz/produção de elastômero ou sistema/poliuretano elastomérico). Resultado real: 46 componentes/sistemas comprovados, zero ADT e zero CAT. A listagem explícita de elastômeros ganhou rota determinística síncrona e streaming: consulta o catálogo estruturado diretamente, mostra total + prévia e não permite que o LLM reinsira auxiliares. Validação real das duas rotas devolveu os mesmos 46, com `model_used: catalogo-estruturado`.

**Testes e ambiente:** ciclos vermelho/verde registrados para o falso positivo lexical, limite global de candidatos, recuperação conversacional, rejeição persistente (inclusive fora da janela de 8 mensagens enviada ao LLM), classificação menção-vs-natureza, rota estruturada síncrona/streaming, sanitização de status e remoção das ferramentas simuladas. Suítes finais: **255/255 backend** e **11/11 frontend**; 5/5 containers healthy.
...
**1. Chave OpenAI configurada e validada** — usuário adicionou saldo na conta OpenAI; `OPENAI_API_KEY` preenchida no `.env`, backend recriado, testado com `gpt-4o-mini`.

**2. Busca híbrida por código de produto e palavra-chave (`327e16a`):** busca puramente semântica (`ollama/nomic-embed-text`, embedding local) trazia produto errado em dois cenários reais — código parecido confundido ("AG 2032" trazia "AG 2062") e aplicação descrita com quase as mesmas palavras do boletim certo não encontrada ("cola para rolha de cortiça" não achava o FLEXX AG 2066, que cita "rolhas de cortiça" quase literalmente). `retrieve_products_context()` ganhou dois índices de texto tokenizados no Qdrant (`filename`, `content`) e busca híbrida: código detectado → match exato no nome do arquivo; termos de conteúdo → match por palavra-chave, exigindo 2+ termos. Aviso explícito no contexto quando um código citado não bate com nada real (evita o LLM apresentar o produto mais parecido como se fosse o pedido). Prompt também passou a diferenciar pedido específico (responde direto) de pedido aberto (só investiga com perguntas quando não há match claro de aplicação).

**3. Estatísticas e listagem do catálogo via ferramentas MCP (`f0561c4`, `8f19bc4`):** perguntas agregadas ("quantos produtos catalogados") e de listagem ("produtos para colchão") não são respondíveis só com RAG (top-k de poucos chunks nunca representa uma categoria inteira — achado real: "colchão" aparece em ~150 arquivos/dezenas de produtos, top-6 de duas variações de pergunta sem nenhuma sobreposição). Duas ferramentas MCP novas com dados reais do Qdrant: `consultar_estatisticas_catalogo` (total do acervo) e `consultar_produtos_por_aplicacao` (lista produtos distintos). **Gap crítico achado e corrigido junto:** o frontend só usa `/api/match/stream`, que nunca chamava ferramentas MCP — nenhuma das duas seria alcançável de verdade pela tela. `stream_pu_matcher_agent` ganhou tool calling (resolve a tool_call numa 1ª chamada sem stream, depois streama a resposta final).

**4. Deduplicação de arquivos redundantes na ingestão (`de7c8f2`):** auditoria da coleção real achou **quase metade dos 10.499 pontos redundante** — uma pasta inteira duplicada ("DOCUMENTAÇÃO DE PRODUTO RESTAURADO 0906", ~4.900 chunks) e ~1.200 pares do mesmo boletim em PDF+DOCX indexados em dobro. `ingest_catalog_directory()` ganhou dois filtros antes de extrair texto/gerar embedding: mesma pasta+nome com extensão diferente → mantém 1 (PDF > DOCX > DOC > TXT); hash de conteúdo normalizado → arquivo idêntico a outro já processado nesta execução é ignorado (reaproveita a reconciliação já existente pra limpar pontos antigos, sem precisar de um passo de deleção dedicado). **Só previne acúmulo futuro** — a coleção já indexada não foi limpa (operação destrutiva em produção, decisão pendente com o usuário, tratada à parte da reingestão em andamento).

**5. Listagem por categoria — prévia de 10 + confirmação, tipo de produto, catálogo inteiro, família (`74450b4`, `5ddf826`, `8f19bc4`..`67b5ba7`):** pedidos sucessivos do usuário foram ampliando `consultar_produtos_por_aplicacao`: (a) por padrão devolve total real + prévia de 10, pergunta se quer a lista completa, só lista tudo se confirmado — validado ao vivo com "produtos para automotivo" (77) e "quantos produtos para automotivo" (mesmo fluxo); (b) passou a cobrir tipo/natureza do produto ("produtos que são colas"), não só aplicação; (c) sem `termo_busca`, lista o catálogo inteiro (1.324 produtos) — antes só dava pra contar; (d) busca por família/código do NOME do produto (ex: "CAT", "TH", "AG" — padrão FLEXX <FAMÍLIA> <NÚMERO>), não só conteúdo.

**6. Dois bugs reais achados validando a família "CAT" ao vivo, ambos corrigidos:** (a) `_produto_do_filepath` reportava pasta administrativa ("FISPQ" solta, "AMOSTRA...") como se fosse produto — corrigido com exclusão por palavra inteira + limite de profundidade de busca (efeito colateral pego na própria correção: excluir "FISPQ" fazia o algoritmo subir até a raiz da árvore e reportar ELA como produto); (b) busca de conteúdo por substring bruta batia em "catálise"/"catalisador" (termos de química reais, nada a ver com a família "CAT"), inflando 36 produtos certos pra 547 errados — corrigido exigindo palavra inteira no conteúdo (frases de várias palavras continuam substring, risco de fragmento é baixo). Achado seguinte, mais sutil: mesmo com palavra inteira, o boletim de OUTRO produto pode citar "FLEXX CAT 90" numa tabela comparativa — um match de conteúdo genuíno, mas que não significa que aquele produto É da família CAT. Resolvido separando a resposta da ferramenta em dois blocos independentes (`por_nome_ou_familia`, `por_aplicacao_ou_tipo`) — pedido explícito do usuário: o agente precisa entender a diferença entre nome de produto e aplicação/segmento, e perguntar ao vendedor quando os dois blocos derem resultados claramente diferentes, em vez de somar/misturar.
